...
export default function WeddingAdminPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="wa-shell">
        ...
        {loading ? <div className="wa-loading"><RefreshCw className="wa-spin" />Preparando tu panel…</div> : (
          <div className="wa-content">
            {view === 'resumen' && <>
              ...
              <div className="wa-dashboard-grid">
                ...
                <section className="wa-panel wa-activity"><div className="wa-panel__heading"><div><p className="wa-eyebrow">Últimos movimientos</p><h2>Actividad y respuestas</h2></div><button onClick={() => setView('respuestas')}>Ver todas</button></div>
                  <div className="wa-activity__list">{guests.length ? guests.slice(0, 4).map((guest) => { const confirmedPasses = guest.respuesta?.pasesConfirmados || guest.pases; return <div key={guest.id}><span className={`wa-status-dot is-${guest.estado}`} /><div><strong>{guest.nombre}</strong><small>{guest.estado === 'confirmado' ? `Confirmó ${confirmedPasses} ${confirmedPasses === 1 ? 'persona' : 'personas'}` : guest.estado === 'rechazado' ? 'Avisó que no asistirá' : guest.vistoEn ? 'Abrió su invitación' : guest.enviado ? 'Esperando su respuesta' : 'Lista para enviar'}</small></div><span className={`wa-status is-${guest.estado}`}>{statusLabel(guest)}</span></div>; }) : <div className="wa-empty-inline">La actividad aparecerá cuando empieces a crear y enviar invitaciones.</div>}</div>
                </section>
            ...
            {view === 'invitados' && <section className="wa-view">
              <div className="wa-view__heading"><div><p className="wa-eyebrow">Tu lista</p><h2>{guests.length} {guests.length === 1 ? 'invitación' : 'invitaciones'} · {stats.passes} {stats.passes === 1 ? 'lugar' : 'lugares'}</h2><p>Buscá, corregí y administrá cada invitación desde un solo lugar.</p></div><div><button className="wa-button wa-button--outline" onClick={() => { setCreateMode('bulk'); setCreateOpen(true); }}><FileUp size={18} />Crear varias</button><button className="wa-button wa-button--primary" onClick={() => { setCreateMode('single'); setCreateOpen(true); }}><Plus size={18} />Nueva invitación</button></div></div>
      ...
      {view === 'invitados' && <button className="wa-mobile-add" onClick={() => setCreateOpen(true)} aria-label="Crear invitación"><Plus /></button>}
