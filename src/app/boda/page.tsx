...
export default function BodaPage() {
  ...
  useEffect(() => {
    // Read code from URL search params
    const params = new URLSearchParams(window.location.search);
    const c = params.get('i') || params.get('inv') || params.get('code') || '';
    if (c) {
      setCode(c);
      fetchGuest(c);
    }
  ...
  const fetchGuest = async (c: string) => {
    try {
      const res = await fetch(`/api/boda/invitados?code=${encodeURIComponent(c)}`);
  ...
  useEffect(() => {
    // Countdown timer for Nov 13, 2026 20:30
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Ocurrió un error al enviar tu confirmación. Intentalo de nuevo.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <header className="site-header" id="siteHeader">
        ...
        <div className="site-header__actions">
          <button className="icon-button" onClick={toggleMusic} type="button" aria-label="Música">
